...
process_frame() {
    ...
    # Initialize prev_errs for new clients to current value to avoid reporting pre-existing errors
    for key in "${!curr_errs[@]}"; do
        local curr_val=${curr_errs[$key]:-0}
        if [[ -z "${prev_errs[$key]+set}" ]]; then
            prev_errs[$key]=$curr_val
            log 3 "Initializing prev_errs for new client $key (${client_pretty_names[$key]:-unknown}) to $curr_val"
        fi
    done
